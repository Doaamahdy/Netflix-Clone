...
const WatchPage = () => {
  ...
  useEffect(() => {
    const getTrailers = async () => {
      try {
        const URL = `/api/v1/${contentType}/${id}/trailers`;
  ...
  useEffect(() => {
    const getSimilarContent = async () => {
      try {
        const URL = `/api/v1/${contentType}/${id}/similar`;
        const res = await axios.get(URL);
  ...
  if(loading) return(<div className="min-h-screen bg-black p-10">
   <WatchPageSkeleton/>
  </div>);
  if(!content) return (<div className="bg-black text-white h-screen">
      <div className="max-w-6xl mx-auto">
        <Navbar/>
        <div className="text-center mx-auto px-4 py-8 h-full mt-40">
            <h2 className="text-2xl sm:text-5xl font-bold text-balancce">Content not found 😥</h2>
        </div>
      </div>
  </div>);
  console.log(content);
  return (
    <div className="bg-black min-h-screen text-white">
      <div className="mx-auto continer px-4 py-8 h-full">
        ...
        {similarContetnt.length > 0 && (
          <div className="mt-12 max-w-5xl mx-auto relative">
            <h3 className="text-3xl font-bold mb-4">
              Similar Movies/TV Shows
            </h3>
            <div className="flex overflow-x-scroll scrollbar-hide pb-4 gap-4 group" ref={sliderRef}>
             {similarContetnt.map((content)=>{
